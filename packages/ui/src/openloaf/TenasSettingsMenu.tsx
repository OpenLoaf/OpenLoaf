...
export type OpenLoafSettingsMenuItem = {
  key: string;
  label: string;
  Icon?: ComponentType<{ className?: string }>;
};

export type OpenLoafSettingsMenuProps = {
  groups: OpenLoafSettingsMenuItem[][];
  activeKey: string;
  isCollapsed?: boolean;
  onChange: (key: string) => void;
  renderItemWrapper?: (item: OpenLoafSettingsMenuItem, button: ReactNode) => ReactNode;
  className?: string;
};

/** Settings menu list with grouped items. */
export function OpenLoafSettingsMenu({
  groups,
  activeKey,
  isCollapsed = false,
  onChange,
