...
type SuggestionData = Omit<TSuggestionData, 'type'> & {
...
export const useResolveSuggestion = (
  ...
  const resolvedSuggestion: ResolvedSuggestion[] = React.useMemo(() => {
      ...
      suggestionNodes
        .flatMap(([node]) => {
          if (TextApi.isText(node)) {
            const dataList = api.suggestion.dataList(node) as SuggestionData[];
    ...
    suggestionIds.forEach((id) => {
      ...
      const nodeData = api.suggestion.suggestionData(
        firstEntry[0]
      ) as SuggestionData | null | undefined;
