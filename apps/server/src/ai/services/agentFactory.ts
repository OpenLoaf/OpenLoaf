...
const SPECIALIST_CONFIGS: Record<string, SpecialistConfig> = {
  ...
  'extractor': {
    ...
    instructions: [
      ...
      '你可以使用以下工具：',
      '- Read: 读取文件内容',
      '- WordInspect / ExcelInspect / PdfInspect: 结构化读取 Word / Excel / PDF',
      '- DocPreview: PPTX 以及 Office 文档的通用预览 / 全量提取',
      '- WebFetch: 获取网页内容',
      '',
      '注意：提取结果使用结构化格式（JSON/表格）呈现。长文档先摘要再详述。',
    ].join('\n'),
    toolIds: ['Read', 'WordInspect', 'ExcelInspect', 'DocPreview', 'PdfInspect', 'WebFetch'],
    maxSteps: 20,
  },
  'canvas-designer': {
    id: 'canvas-designer',
