...
const SPECIALIST_CONFIGS: Record<string, SpecialistConfig> = {
  ...
  'browser': {
    ...
    instructions: [
      ...
      '- browser-click: 点击元素',
      '- browser-fill: 填写表单',
      '- BrowserSnapshot: 快照+截图',
      '- WebSearch: 搜索网页',
      '',
      '注意：操作网页时注意加载等待，避免操作未渲染的元素。',
    ].join('\n'),
    toolIds: ['browser-navigate', 'browser-click', 'browser-fill', 'BrowserSnapshot', 'browser-read', 'WebSearch'],
    maxSteps: 40,
  },
  'data-analyst': {
    id: 'data-analyst',
  ...
  'extractor': {
    ...
    instructions: [
      ...
      '你可以使用以下工具：',
      '- Read: 读取文件内容',
      '- WordInspect / ExcelInspect / PdfInspect / PptxInspect: 结构化读取 Word / Excel / PDF / PPTX',
      '- DocPreview: Office 文档的通用预览 / 全量提取',
      '- WebFetch: 获取网页内容',
      '',
      '注意：提取结果使用结构化格式（JSON/表格）呈现。长文档先摘要再详述。',
    ].join('\n'),
    toolIds: ['Read', 'WordInspect', 'ExcelInspect', 'DocPreview', 'PdfInspect', 'PptxInspect', 'WebFetch'],
    maxSteps: 20,
  },
  'canvas-designer': {
    id: 'canvas-designer',
