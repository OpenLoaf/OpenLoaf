...
- `apps/server/src/ai/tools/browser/playwright/tools.ts`

该文件负责：

- 导出所有 `playwright*Tool`（供 `apps/server/src/ai/tools/browser/tools.ts` 注册）
- 组合调用本目录的模块完成具体行为
...
2. 在 `apps/server/src/ai/tools/browser/playwright/tools.ts` 里新增导出的 tool，并尽量复用 `withCdpPage`
3. 在 `apps/server/src/ai/tools/browser/tools.ts` 注册 toolId -> tool
