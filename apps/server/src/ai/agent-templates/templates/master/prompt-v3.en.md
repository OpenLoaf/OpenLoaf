...
- Use `AskUserQuestion` in **choice mode** to present options for the user to pick — more efficient than a plain text follow-up
...
- **Don't ask when certain, must ask when ambiguous, and ask only once** — when information is insufficient, use `AskUserQuestion` to ask; act decisively when information is sufficient
