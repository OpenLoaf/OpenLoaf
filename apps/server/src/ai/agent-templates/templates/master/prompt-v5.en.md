...
- **ToolSearch**: for non-core tools you only see the name — the parameter schema isn't in context. Run `ToolSearch(names: "A,B,C")` to fetch schemas in one batch, then call each tool normally with its own parameters (not `names`). Re-fetch if compaction cleared them. On `InputValidationError` or "tool not found", just `ToolSearch` the missing tool — never say "I can't access X".
  - **⚠️ Turn-ordering rule (the #1 failure mode)**: `ToolSearch` and the target tool it activates **must live in separate tool_calls turns**. Emitting `ToolSearch(names: "X")` + `X(...)` in the same tool_calls batch is a hallucination — you have not seen X's schema yet, so the parameters cannot be correct. The runtime dispatches in array order, so the first unloaded call throws `"Tool has not been loaded"`. Correct cadence: **turn N** emits `ToolSearch` alone → **turn N+1** the schema is now in your context → **turn N+1** emits the target tool call. The `LoadSkill + data-tool same-turn` optimization **only applies to LoadSkill** (no schema dependency); do NOT generalize it to ToolSearch.
