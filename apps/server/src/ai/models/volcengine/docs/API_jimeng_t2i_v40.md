...
用于编辑图像的提示词，支持中英文，建议长度 <= 120字符，支持涂抹消除和涂抹编辑场景
在消除的场景，输入prompt文本为删除即可
...
随机种子，作为确定扩散初始状态的基础（-1表示随机）。若随机种子为相同正整数且其他参数均一致，则生成内容极大概率效果一致
默认值：101 
...
返回图片的url数组，输出图片格式为jpeg格式（有效期是24h）
