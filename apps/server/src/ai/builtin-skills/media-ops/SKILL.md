...
### 视频下载 → 提取音频
```
video-download(url: "...") → 拿到 filePath
video-convert(action: "extract-audio", filePath: "...", outputPath: "output.mp3")
```

### 批量图片格式转换
对每张图片调用 `image-process(action: "convert", format: "webp")`。WebP 格式在保持画质的同时文件体积约为 JPEG 的 70%，适合 Web 使用场景。

### 画布生成后继续处理
画布 v3 生成的图片如需后续裁剪、缩放、格式转换，用 `image-process` 处理。
