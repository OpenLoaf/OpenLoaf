...
OpenLoaf 中一切文件操作都发生在**项目上下文**中。项目 = 一个磁盘根目录 + OpenLoaf 侧栏中的注册记录。所有文件工具（`Read`、`Edit`、`Glob`、`Grep`、`Bash`）的路径都**相对于项目根目录**，无法越界——这是安全沙箱设计。
...
**探索项目**：`project-query { mode: "get" }` → `Glob { pattern: "**/*" }` → `Grep` → `Read`

**创建项目**：`project-query { mode: "list" }` → `project-mutate { action: "create", ... }` → `Write` 写初始文件 → `Bash` 初始化依赖
...
**混淆 remove 与删除** — `remove` 只摘注册不碰磁盘。原因：防止误删用户代码。如果用户确实要删磁盘文件，需显式用 `Bash rm -rf`。
