...
description: >
  ...
  异步执行、支持 cron/interval/once 调度和审批流。触发词：定时、cron、
  每天/每周/每月做某事、自动化、周期性、后台执行、异步任务、看板任务、
  审批、调度、延后提醒。**不负责会话内的进度条追踪（那是 runtime-task-ops）**，
  也不负责一次性计划审批（那是 SubmitPlan）。用户说"创建任务"时先判断意图：
  要入库定时/审批才用这个；只是想看实时进度用 runtime-task-ops。
---
