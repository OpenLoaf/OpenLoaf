...
2. **scheduleAt** → 用 `time-now` 拿当前时间和时区，推算"明天早上"为次日 09:00，转换为 ISO 8601
...
**scheduleAt 已过期** — once 类型的 `scheduleAt` 必须是未来时间。原因：已过期的时间无法调度，会被系统拒绝。创建前务必用 `time-now` 核实当前时间。
