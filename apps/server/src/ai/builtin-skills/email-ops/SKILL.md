...
4. 草拟回复
5. ⚠️ request-user-input        → 把草稿展示给用户确认
...
2. 根据用户意图草拟邮件
3. ⚠️ request-user-input        → 展示完整邮件让用户确认
4. send                         → 确认后发送
```
...
3. 通过 `request-user-input` 让用户确认后 `send`
...
1. **发邮件前没调 `request-user-input`**：这是最严重的错误。永远不要自作主张发送邮件。
