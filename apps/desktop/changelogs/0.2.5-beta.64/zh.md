...
> 0.2.5-beta.63 因 `keytar` 的 `.node` 原生二进制让 esbuild 失败、且未被复制进 desktop 打包产物，CI 失败。本版本修复打包问题并重发。
...
## 🐛 构建修复（相对 0.2.5-beta.63）

- 把 `keytar` 加入 server `esbuild` external 列表，避免把 `.node` 二进制塞进 bundle。
- 把 `keytar` 加入 desktop `forge.config.ts` 的 `NATIVE_DEP_ROOTS`，确保打包产物的 `node_modules` 中包含该包及其 native binding。
