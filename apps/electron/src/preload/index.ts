 ...
 * - 用于 Web UI 在发送 /chat/sse 时携带 appId，让 server 能精确调度到当前桌面端。
 */
const appId: string = (ipcRenderer.sendSync('teatime:get-app-id-sync') as string) || '';
...
contextBridge.exposeInMainWorld('teatimeElectron', {
  appId,
  getAppId: (): Promise<string> => ipcRenderer.invoke('teatime:get-app-id'),
  // 请求主进程在独立窗口中打开外部 URL。
