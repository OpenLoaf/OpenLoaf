/**
 * office-create/033: PDF 旋转 — 旋转指定页。
 *
 * 附件一份 PDF，让 AI 把第 1 页顺时针旋转 90 度。正确路径是
 * JsSandbox（pdf-lib rotate，angle=90），一步完成。
 * 断言：调用了 JsSandbox，回复确认旋转完成。
 */
...
it('office-create-033 — PDF 旋转：JsSandbox 第 1 页顺时针旋转 90 度', async () => {
  ...
  const meta = {
    ...
    description: 'JsSandbox 把 PDF 指定页旋转 90 度',
    tags: ['jssandbox', 'rotate', 'pdf'],
  }
  ...
  // 工具链效率：理想 ≤4 步（LoadSkill + JsSandbox pdf-lib rotate）
  ...
  const judgment = await aiJudge({
    ...
    criteria:
      ...
      '2) 回复为空但工具调用包含 JsSandbox（rotate 已执行）。',
