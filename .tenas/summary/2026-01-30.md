...
- **原生助手构建**：完善 macOS `CalendarHelper.swift`、`tenas-calendar` 及 `buildCalendarHelper.mjs`，新增 `package.json` 依赖；补充 `electron.d.ts` 类型定义。
...
- **macOS 语音助手**：构建 `tenas-speech`。

### 5. 跨平台原生扩展与 API 增强
- **Windows 日历支持**：更新 `Program.cs`、`TenasCalendar.csproj`，完善 Windows 原生日历集成。
