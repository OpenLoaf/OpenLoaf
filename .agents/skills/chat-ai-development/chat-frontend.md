...
**方案 B**: 需要自定义渲染时，在 `message/tools/` 创建组件，然后在 `message/tools/tool-registry.ts` 注册组件：
...
- `leafMessageId` — 当前叶节点
- `useChatBranchState()` 统一维护 `messages / leafMessageId / branchMessageIds / siblingNav / errorMessage` 的服务端快照，并负责 `getChatView` 的初始加载、缓存清理、手动刷新

**切换流程**: 点击 prev/next → `switchSibling()` → 服务端 `getChatView()` 返回新分支快照 → 覆盖本地 messages

### Snapshot 收口规则

- `ChatCoreProvider` 不再直接维护独立的 `sessionErrorMessage + branchQuery + leaf/sibling setters`
- 初始历史加载：`useChatBranchState().branchQueryData` → `applySnapshot()` → 覆盖本地 `chat.messages`
- 手动刷新：`refreshSnapshot()` 返回服务端 canonical snapshot，供切分支 / 删除后刷新复用
- 动作返回：高频动作应优先直接返回 canonical snapshot（当前 `deleteMessageSubtree` 已改为 mutation 内直接返回 snapshot），避免前端动作后再追加一次 `getChatView`
- 流完成收口：`ChatCoreProvider.onData` 会消费 `data-branch-snapshot`，统一调用 `commitServerSnapshot(...)`；普通发送、retry、resend、审批续接完成后，都应以服务端 snapshot 覆盖本地临时消息链
- 元数据刷新：`refreshBranchMeta()` 只更新 `leafMessageId / branchMessageIds / siblingNav`，不覆盖当前 messages
- fallback 规则：`needsBranchMetaRefreshRef` 只保留给“流结束但没收到 `data-branch-snapshot`”的异常场景，不能再作为 retry/resend 的主路径
- 会话切换：必须同时 `resetSnapshot()` + `clearCachedView()`，避免复用 Infinity stale 的旧 `getChatView` 结果

## Input Area

| 文件 | 作用 |
|------|------|
| `input/ChatInput.tsx` | 主输入框（ContentEditable 富文本、mention、拖拽上传） |
| `input/ChatInputEditor.tsx` | ContentEditable 编辑器（芯片渲染、值↔DOM 双向转换） |
| `input/ChatCommandMenu.tsx` | 斜杠菜单（/ 触发，显示可用技能列表） |
...
### ChatInput 内联芯片（Chip）渲染规范

ChatInput 使用 ContentEditable 编辑器（`ChatInputEditor.tsx`），通过 `valueToHtml` 将纯文本值渲染为带内联芯片的 HTML。所有芯片遵循以下规则：

**芯片类型**

| 类型 | 值格式 | CSS 类 | 颜色 | 示例 |
|------|--------|--------|------|------|
| 文件引用 | `@{path}` | `ol-mention-chip` | 蓝色 (blue-200/blue-700) | `@{src/index.ts}` |
| 技能命令（新） | `/skill/[originalName\|displayName]` | `ol-skill-chip` | 紫色 (purple-200/purple-700) | `/skill/[ai-test-development\|AI 测试开发]` |
| 技能命令（旧） | `/skill/name` | `ol-skill-chip` | 紫色 (purple-200/purple-700) | `/skill/frontend-testing` |

**技能命令格式（重要）**

技能命令使用方括号格式区分原始名和显示名：
- **有翻译**: `/skill/[原始名|翻译名]`，芯片显示翻译名，后端提取原始名用于匹配
- **无翻译**: `/skill/[原始名]`，芯片显示原始名
- **旧格式（兼容）**: `/skill/name`，仅支持 ASCII 字符

核心函数 `buildSkillCommandText(originalName, displayName?)` 在 `chat-input-utils.ts` 中生成命令文本。

**芯片渲染条件（重要）**

新格式 `/skill/[...]` 有明确的方括号边界，无需 lookahead。旧格式 `/skill/name` 仍需紧跟**空白字符**或**非 ASCII 字符**才会渲染为芯片。

- `/skill/[ai-test\|AI 测试] 讲什么` → ✅ 芯片（显示"AI 测试"）+ 文本
- `/skill/frontend-testing 讲什么` → ✅ 旧格式芯片 + 文本
- `/skill/frontend-testingabc` → ❌ 纯文本（旧格式无边界）

**隐形分隔符（重要）**

芯片渲染时，`valueToHtml` 将紧跟的空格吸收进 `data-token` 属性中，HTML 不再渲染这个空格。用户视觉上看不到间隙，但 `domToValue` 从 `data-token` 读回时空格仍在，保证值 round-trip 一致。

实现要点：
1. `data-token` 存储完整 token + 尾部空格（如 `"/skill/[xxx|yyy] "`）
2. `lastIndex` 跳过被吸收的空格
3. `domToValue` 直接读 `el.dataset.token`，空格自动还原

**新增芯片类型时的清单**

1. 在 `ChatInputEditor.tsx` 中：
   - 添加 CSS 类常量和样式到 `CHIP_STYLES`
   - 扩展 `valueToHtml` 正则，添加匹配分支
   - 新格式优先用方括号界定边界，旧格式使用 `(?=\s|[^\x00-\x7F])` lookahead
   - 将尾部空格吸收进 `data-token`，HTML 不渲染
   - 扩展 `domToValue` 识别新芯片类（已有通用逻辑，通常只需加 class 判断）
   - 扩展 `isEmpty` / `updateDomEmpty` 的选择器
2. 在 `message/text-tokenizer.ts` 中：
   - 添加新 token 类型到 `ChatTextToken`（skill 类型已有 `displayName?` 可选字段）
   - 在 `splitCommandSegments` 中添加匹配（新格式 + 旧格式 `(?=\s|[^\x00-\x7F]|$)`）
3. 在 `message/ChatMessageText.tsx` 中：
   - 添加新 token 类型的渲染分支
   - skill 类型优先显示 `segment.displayName`，回退到 `segment.value`（originalName）
   - 点击行为通过共享函数实现（如 `openSkillInStack`，定义在 `setting/skills/skill-utils.ts`）
4. 如有点击交互，在 `ChatInputEditor` 添加 `onXxxChipClick` 回调，在 `ChatInput` 中接入
...
| Hook | 文件 | 作用 |
|------|------|------|
| `useChatBranchState` | `hooks/use-chat-branch-state.ts` | 服务端 branch snapshot 管理（`getChatView` 查询 + `messages/leaf/branch/sibling/error` 收口） |
...
| 芯片正则不加 lookahead，用户删空格后名称被破坏 | 新格式用 `[...]` 方括号界定边界；旧格式用 `(?=\s\|[^\x00-\x7F])` 确保边界；text-tokenizer 中额外允许 `\|$` |
| 芯片后空格对用户可见 | `valueToHtml` 必须将尾部空格吸收进 `data-token`，HTML 中不渲染该空格 |
| 新增芯片类型但未同步 text-tokenizer | 输入框和消息气泡两处都需要处理，否则发送后芯片消失 |
| 技能命令用翻译名匹配后端 | 前端命令始终用 `originalName` 生成（`buildSkillCommandText(originalName, displayName)`），显示用 `displayName`；后端 `SkillSelector` 优先匹配 `originalName` |
| `SkillSummary` 缺 `originalName` | API schema 已含 `originalName` 字段，前端类型需同步声明 |
