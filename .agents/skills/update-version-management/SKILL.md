...
Tag message 使用 Step 3 生成的**英文版**更新日志（去掉 frontmatter）。如果有 changelog 文件，直接读取；否则用 commit 摘要。

```bash
# 从 changelog 文件读取（推荐）
git tag -a server-v{currentVersion} -F apps/server/changelogs/{currentVersion}/en.md
git tag -a web-v{currentVersion} -F apps/web/changelogs/{currentVersion}/en.md

# 或手动编写英文摘要
git tag -a server-v{currentVersion} -m "Add streaming API support
...
> **Tag message 规范：** 全部使用英文，内容为本次更新的变更摘要（与 changelog 一致）。不使用 `release: xxx` 这类占位消息。
