...
- `publish-desktop.yml` 从 `desktop@*` tag 提取版本号后，应直接改写 `apps/desktop/package.json` 的 `version`
- 不要在 CI 里用 `npm version` 回写 Desktop 版本；当前仓库含 `catalog:` 依赖，runner 上会报 `EUNSUPPORTEDPROTOCOL`
