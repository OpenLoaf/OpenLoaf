# Server AI 重构方案（V2：参考 Stagehand + MasterAgent/SubAgent）

> 适用范围：仅针对本仓库 `apps/server` 的 AI 核心重构设计（包含与 AI 强绑定的 UI/stream 协议：`open-url`、sub-agent SubAgent stream 合并、断点续传/强制中断、审批交互）。  
> 目标：参考 `docs/stagehand-core-agent.md` 中 Stagehand 的 AI SDK 组织方式（instructions 构建、调用前压缩、tools 管理），并结合 `docs/mutli-agent.md` 的 MasterAgent/SubAgent 原则，重新设计 server 侧 AI 架构。  
...
1. **Agent 管理**：从“硬编码单一 MasterAgent”升级为“可注册、可配置、可组合”的多 agent（MasterAgent/SubAgent）体系；最好能与 DB 中的配置/定义打通。
...
2. **AI SDK 流式入口与自定义断点续传/强制中断必须保留**  
   ...
   - 强制中断：`POST /chat/sse/:id/stop` → abortSignal（同一 abortSignal 必须能中止 MasterAgent 与 SubAgent）。
3. **SubAgent 被 MasterAgent 当作 tool 调用，且 SubAgent 输出必须 merge 到同一个 SSE**  
   - 输出形式：SubAgent 的 chunk 必须以“stream tool output”的方式写入主 SSE（而不是新开 assistant message / 另开 SSE/WS）。  
   - 语义以 ToolDef 为准：`packages/api/src/types/tools/subAgent.ts`（描述里明确 “流式输出合并到当前对话中”）。
4. **审批交互走 SSE（停止/续跑）**  
   - 当遇到 `needsApproval` 的工具调用时：AI SDK 会输出 `tool-approval-request` chunk 并结束本次生成（不执行工具）。  
   - 用户在页面上完成审批后：前端调用 `addToolApprovalResponse` 写回审批结果，然后触发一次新的 `sendMessage()`；server 继续执行并输出 tool result。  
   - 审批续跑不需要 PendingToolApproval：以“最后一条 message + parentMessageId 追溯历史链”为唯一事实来源。
...
- SSE 入口：`apps/server/src/modules/chat/ChatSseRoutes.ts`
  ...
  - 前端只发送 lastMessage；server 通过 `parentMessageId` 在 DB 中追溯完整历史链后再调用模型
  - 断线续传：`apps/server/src/modules/chat/StreamStoreAdapter.ts`（Keyv + EventEmitter + buffer 上限）
- Agent：`apps/server/src/ai/agents/masterAgent/masterAgent.ts`
...
- server 侧工具实现：`apps/server/src/ai/tools/*`
  ...
  - sub-agent 工具已启用：`apps/server/src/ai/tools/delegation/subAgentTool.ts`
...
## 2. 设计参考：Stagehand 与 MasterAgent/SubAgent 的可迁移点
...
### 2.2 `docs/mutli-agent.md` 的 MasterAgent/SubAgent 核心原则

- Lead agent（MasterAgent）负责：拆解、分配预算、协调与合并结果。
- SubAgent 负责：在自己上下文里“花 token”，并把输出**压缩**成高密度摘要返回。
- 长会话需要 memory：把计划/阶段结论写入外部存储，避免上下文截断导致失忆（本次 memory 需要结构化）。
- 减少“传话损耗”：SubAgent 输出要结构化、可检索；长内容后续落到 ArtifactStore（存本地或 OSS 文件系统，本次放最后做）。

---

## 2.3 本项目必须继承的 AI SDK Streaming 组织方式（V2 的硬约束）

> 这部分不是“可选最佳实践”，而是已经在生产链路里跑通的基建；V2 需要复用其机制并扩展到 SubAgent。
...
- **断点续传/强制中断**：`apps/server/src/modules/chat/StreamStoreAdapter.ts`
  ...
  - stop 时额外输出 `data-manual-stop`，用于 UI 显示“手动中断”工具卡片
- **V2 扩展点**：SubAgent 的输出需要“像 sub-agent ToolDef 描述的一样”被 merge 到同一个 writer，并且以“stream tool output”的形式输出；SubAgent 的 chunk 也必须进入同一个 `StreamStoreAdapter` buffer，才能实现“断线续传覆盖 SubAgent 输出”。
...
5. **MasterAgent/SubAgent 的运行时模型固定**：SubAgent 由 MasterAgent 通过 `sub-agent` 这个“工具”调用；SubAgent 的输出以“stream tool output”形式合并进同一条 SSE，并被当前 AI 消息的 `parts` 持久化；主 agent 只需要把 SubAgent 的最终结构化结果用于合并决策。
...
建议输出形态（概念）：
- `AgentSpec { id, kind(masterAgent|subAgent), model, toolPackKeys, promptProfile, limits }`
...
### 4.1.1 SubAgent 作为 Tool 的运行与落库模型（关键行为）

> 这部分将 `docs/mutli-agent.md` 的理念落到“本项目的 SSE/DB/UI 现实约束”上。

**约束 A：SubAgent 被当作 tool 调用（调用点在 MasterAgent 的 tool-loop 内）**
- ToolDef：`packages/api/src/types/tools/subAgent.ts`（`id: "sub-agent"`）
- server 实现：`apps/server/src/ai/tools/delegation/subAgentTool.ts`（内部使用 `apps/server/src/ai/runners/subAgentRunner.ts`）

**约束 B：SubAgent 的 stream 必须 merge 到当前 SSE（而不是另开一条 SSE/WS）**
- SubAgent 执行同样使用 AI SDK 的 `createAgentUIStream`
- SubAgent 输出通过主请求的 `writer.merge()` 合并到同一个 `createUIMessageStream` 内（入口仍然是 `apps/server/src/modules/chat/ChatSseRoutes.ts`）
- **关键规则**：SubAgent 的文本 chunk 必须以“stream tool output（sub-agent tool 的输出流）”的方式写入 SSE（带 `toolCallId`），从 UI 角度它是一段“工具的流式输出”，而不是新的 assistant message。
- 目的：前端看到的是“同一条 SSE 流”里出现 sub-agent 工具的实时输出；断线续传也能完整回放（`apps/server/src/modules/chat/StreamStoreAdapter.ts`）

**约束 C：SubAgent 输出持久化策略（最小闭环）**
- SubAgent 的“流式输出内容”不再单独落一条 `ChatMessage(role=assistant)`；而是作为本轮 AI 消息的 tool parts 随 SSE 一起落库/回放（更贴近“stream tool output”的定位，减少消息树复杂度）。
- SubAgent 的“最终结构化结果”由 `sub-agent` 工具的最终返回值承载（例如 `{ summary, evidence, nextSteps }`），供 MasterAgent 合并；该返回值同样作为 tool part 落库。
- 如需可观测性：可以在 `metadata` 中记录 `{ subAgentName, toolCallId }`（不强制新增 schema/表）。

**约束 D：SubAgent 在 UI 的呈现方式（工具样式 + 自动收缩）**
- SubAgent 输出以 `sub-agent` 工具卡片呈现（tool message + tool output stream），而不是独立 assistant bubble。
...
- `Strategy`：
  - MasterAgent：拆解/预算/委派/合并规则
  - SubAgent：只做子任务、严格输出 schema、输出尽量短且结构化
- `Completion Contract`：完成时必须包含哪些字段（例如结论/依据/下一步）

#### TaskPromptBuilder（任务描述规范化）

- MasterAgent 接到用户消息后，先生成“可委派的子任务描述”（含 scope/output schema/stop condition/allowed tools/budget）
- SubAgent 接到子任务后，严格按 schema 输出（建议 JSON）
...
- MasterAgent/SubAgent 只把摘要与引用（`artifactId`）带回主上下文，避免 history 膨胀
...
- MasterAgent 每个阶段结束写一次 memory（减少长会话截断影响）
...
写入规则（简化版）：
- 只允许 MasterAgent 写入；SubAgent 只产出结构化 `evidence/summary`，由 MasterAgent 归并到 memory。
...
建议的包（概念）：
- `pack:master-agent-base`：只含委派/读取/汇总相关工具（避免 MasterAgent 过度执行细节）
- `pack:sub-agent-research`：webFetch/webSearch/fileRead 等（只读为主）
- `pack:sub-agent-project`：project CRUD（含审批）
- `pack:ui-control`：所有 UI side-effect 工具（必须走审批/模式开关）
  - 必须包含并保持 `open-url`：ToolDef `packages/api/src/types/tools/browser.ts`，实现 `apps/server/src/ai/tools/ui/openUrl.ts`
...
规则（停止/续跑）：
1. 当模型调用到 `needsApproval` 的工具时，AI SDK 在 SSE 中输出 `tool-approval-request` chunk，并结束本次生成（HTTP 连接结束）。  
2. 前端展示审批 UI。用户确认/拒绝后，前端调用 `addToolApprovalResponse({ id: approvalId, approved, reason? })` 写回审批结果，然后触发一次新的 `sendMessage()`（会发起新的 SSE 请求）。  
3. server 只接收 lastMessage；通过 `parentMessageId` 追溯 DB 历史链后继续执行工具并输出 tool result，随后模型继续生成文本（多 step）。

> 关键点：审批是“外部交互断点”，不是在同一条 SSE 上阻塞等待（避免连接超时/页面关闭导致状态丢失）。

---

## 5. MasterAgent/SubAgent 协议（必须先定规矩）

### 5.1 MasterAgent 的“委派规则/预算规则”（建议写进 prompt）

- 何时委派：需要外部检索/多源验证/长工具链/需要把“细节探索”从主上下文隔离出去
- subagent 数量限制：按复杂度 1~N（给出明确阈值）
- **执行方式：本期 SubAgent 一律同步串行（按顺序一个个跑），未来再升级并行**
- 每个 SubAgent 的预算：最大步数、最大工具调用
- 防重复：每个 SubAgent 的 scope 必须互斥（例如按来源/子主题分工）

### 5.2 SubAgent 的输出规范（建议 JSON schema）
...
> 重要：SubAgent 不要输出长篇正文；短期可以依赖工具结果自身的截断/压缩策略控制体积，长期把长内容落到 ArtifactStore 并只返回 `artifactId`。

### 5.3 TimeSubAgent（测试用 SubAgent）定义（用于打通全链路）

> 目标：用一个“几乎零依赖”的 SubAgent，把 MasterAgent→sub-agent tool→SubAgent stream merge→落库→UI 展示整条链路跑通。

- SubAgent 名称建议：`time`（对应 sub-agent 调用参数 `name`）
- SubAgent 行为：只调用一次 `time-now` 工具并返回当前时间
  - ToolDef：`packages/api/src/types/tools/system.ts`（`timeNowToolDef`，`id: "time-now"`）
  - 工具实现：`apps/server/src/ai/tools/system/timeNow.ts`
- SubAgent 输出要求：
  - 对 web：流式输出（让 UI SubAgent 卡片能看到实时生成）
  - 对 MasterAgent：工具返回值只给“最终总结”（例如 `summary: "现在是 ..."`）

---

## 6. 目标运行态时序图

### 6.1 主对话（MasterAgent tool-loop + 委派 SubAgent + SubAgent stream merge）

```mermaid
sequenceDiagram
participant UI as Web/Electron
participant SSE as /chat/sse
participant MR as MasterAgentRunner
participant MA as MasterAgent
participant TR as ToolRegistry
participant SA as SubAgentTool
participant SR as SubAgentRunner
participant RA as SubAgent
participant MS as MemoryStore
participant AS as ArtifactStore
participant DB as Prisma(DB)

UI->>SSE: POST /chat/sse(sessionId, userMsg, tabId, clientId)
SSE->>MR: buildRunContext(loadHistory+memory+uiState)
MR->>MA: run(tool loop, streaming)
MA->>TR: call tool (e.g. project-list/web-search/sub-agent)
TR-->>MA: tool result

MA->>SA: sub-agent(name, task, budget)
SA->>SR: run sub-agent with slim context + same abortSignal
SR->>RA: run(tool loop, streaming)
RA-->>SSE: stream tool output (sub-agent tool output stream, writer.merge)
RA->>TR: webFetch/fileRead/... (domain tools)
RA->>AS: store long content (optional / last milestone)
SR-->>SA: subAgentResult(summary + evidence + nextSteps)
SA-->>MA: condensed result

MA-->>SSE: stream answer chunks
...
```mermaid
flowchart TD
A[HistoryLoader: DB主链(基于 parentMessageId)] --> B[ContextAssembler: 拼接 memory + uiState]
...
participant OA as Agent(tool-loop)
participant TR as ToolRegistry
participant SSE2 as /chat/sse (after approval)

UI->>SSE1: POST /chat/sse(sessionId, userMsg, ...)
SSE1->>OA: run (streaming)
OA->>TR: call tool(needsApproval=true)
TR-->>SSE1: emit tool-approval-request(approvalId,toolCallId,toolName,input)
SSE1-->>UI: SSE close (stop after approval request)

UI->>UI: user approves/denies
UI->>SSE2: POST /chat/sse(lastMessage include tool approval response)
SSE2->>OA: load history by parentMessageId then continue tool loop
...
3. **启用 sub-agent（同步串行 + stream tool output）**  
   - SubAgent 串行执行（先求稳定）
   - SubAgent 输出以 tool output stream 合并进主 SSE（不另存 ChatMessage）

4. **ApprovalGateway（SSE 停止/续跑）**  
   - `needsApproval` 输出 `tool-approval-request` 后结束 SSE（不执行工具）
   - 前端 `addToolApprovalResponse` 后再 `sendMessage()` 触发下一次生成（执行工具并继续）
...
1. SubAgent 先采用同步串行（未来再改并行）。
2. ArtifactStore 需要：后续存到本地或 OSS 文件系统；本次放最后开发（先把接口/协议在文档定好）。
3. MemoryStore 采用结构化 schema（可检索、可增量合并）。
4. 审批通道走 SSE：输出 `tool-approval-request` 后结束当前 SSE；前端 `addToolApprovalResponse` 后再 `sendMessage()` 触发下一次生成。
...
- [x] 2) MVP：可用的 tool-loop + SubAgent stream merge + 审批停点（最小闭环）
  - [x] 2.1) 实现 ToolRegistry（以 `ToolDef.id` 为 key）+ ToolPacks + Policy（包含 `needsApproval` 元信息）
  - [x] 2.2) 实现 `sub-agent` 工具（server 侧）：SubAgent 同步串行执行，并把 SubAgent 输出 merge 到主 SSE，作为 stream tool output 输出
  - [x] 2.3) 打通 SSE 审批停点：遇到 `needsApproval` 输出 `tool-approval-request` 后结束 SSE（不执行工具）
  - [x] 2.4) 实现审批续跑：前端 `addToolApprovalResponse` 后触发 `sendMessage()`，server 仅接收 lastMessage 并通过 `parentMessageId` 追溯历史后继续执行
  - [x] 2.5) 保持 `open-url` 业务规则不变：允许调整 ToolDef/实现/注册，但确保 UI 行为与 tab snapshot 回传链路一致
...
- [ ] 4) MemoryStore（结构化）接入与压缩策略（让长会话可持续）
  ...
  - [ ] 4.2) 接入 MasterAgent 的“阶段结束写入”与检索（把关键决策/待办/事实写入）
