...
- `tenas-remote://ssh-remote+host/`（SSH）
- `tenas-remote://dev-container+id/`（容器）
- `tenas-remote://wsl+Ubuntu/`（WSL）
...
### 文件删除
- 删除文件后，清理：
  - `.tenas/yjs/<fileId>.wal`
  - `.tenas/yjs/<fileId>.lock`
...
sequenceDiagram
  ...
  Web->>WS: join(fileId)
  WS->>Provider: readWal(.tenas/yjs/<fileId>.wal)
...
sequenceDiagram
  ...
  Web->>WS: update
  WS-->>Web2: update
  WS->>Provider: appendWal(.tenas/yjs/<fileId>.wal)
...
- [ ] 搜索/索引落地：在 server 接入 `rg --json` 扫描与 `.tenas/index/` 更新策略，并在 Web 搜索入口调用 `fs.search`。
